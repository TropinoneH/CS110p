...
#pragma omp parallel for
        for (i = 2; i < N - 3; i += 2) {
            ...
            odd_next[(i + 1) * half + 1] = 0.0625 * (odd[(i - 2) * half + 1] + p[(i - 1) * N] + odd[i * half + 2] + p[(i + 1) * N] + odd[(i + 2) * half + 1]) +
                                           0.125 * (odd[(i - 1) * half + 1] + odd[(i + 1) * half + 1]) + 0.1875 * (odd[i * half + 1]) + 0.25 * p[i * N];
...
#pragma omp parallel for
        for (j = 2; j < half - 1; ++j) {
            ...
            odd_next[(N - 2) * half + j - 1] = 0.0625 *
                                               (odd[(N - 4) * half + j] + odd[(N - 2) * half + j - 1] + odd[(N - 2) * half + j + 1] +
                                                p[(N - 1) * N + (j - 1) * 2] +
                                                p[(N - 1) * N + j * 2]) + 0.125 * (odd[(N - 3) * half + j - 1] + odd[(N - 3) * half + j]) +
                                               0.1875 * odd[(N - 2) * half + j] + 0.25 * p[(N - 1) * N + 2 * j - 1];
        }


        odd_next[half + 1] =
                0.0625 * (p[2] + odd[half + 2] + p[2 * N] + odd[3 * half + 1]) + 0.125 * (odd[half + 1] + odd[2 * half + 1]) + 0.25 * (p[1] + p[N]);
        // only for N is even
