...
logisim_location = "/usr/local/applications/logisim/"
...
class TestCase:
    ...
    def __call__(self, filename):
        ...
        proc = subprocess.Popen(
            ["java", "-jar", logisim_location, "--tty", "table", self.circfile],
            stdin=stdinf,
...
tests = [
    (
        "Exercise 2 (Sub-Circuits) test",
        "student_output/ex2_test.out",
        TestCase(
            os.path.join(file_locations, "circ_files/ex2_test.circ"),
            os.path.join(file_locations, "reference_output/ex2_test.out"),
            "",
        ),
    ),
    (
        "Exercise 3 (Add Machine) test",
        "student_output/ex3_test.out",
        TestCase(
            os.path.join(file_locations, "circ_files/ex3_test.circ"),
            os.path.join(file_locations, "reference_output/ex3_test.out"),
            "",
        ),
    ),
    (
        "Exercise 4 (FSM) test",
        "student_output/ex4_test.out",
        TestCase(
            os.path.join(file_locations, "circ_files/ex4_test.circ"),
            os.path.join(file_locations, "reference_output/ex4_test.out"),
            "",
        ),
    ),
]
